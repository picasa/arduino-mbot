...
void setup()
{
  ...
  buzzer.tone(NTD4, 200); 
  delay(200);
  rgb.clear();
  
  rgb.setColor(0, 100, 0);
  buzzer.tone(NTD3, 200);
  delay(200);
  rgb.clear();
...
void modeB()
{
  uint8_t distance = ultrasonic.distanceCm(50);
  static long time = millis();
  randomSeed(analogRead(6));
  uint8_t randNumber = random(2);
  
  if (distance > 25 || distance == 0) Forward();
  else if (distance > 15) {
    switch (randNumber)
    {
      case 0:
  ...
  switch(device){
   ...
   case LEDMATRIX:{
     ...
     if(action==1){
            ...
            matrix.clearScreen();
            matrix.setColorIndex(color);
            matrix.setBrightness(brightness);
            matrix.drawStr(px,py,s);
         }else if(action==2){
           int brightness = readBuffer(8);
            int dw = readBuffer(9);
            int px = readShort(10);
            ...
            matrix.setColorIndex(1);
            matrix.setBrightness(brightness);
            matrix.showClock(hours,minutes,point);
     }
   }
   break;
   case LIGHT_SENSOR:{
  ...
  ***************************************************/
  ...
  switch(device){
   case  ULTRASONIC_SENSOR:{
     ...
     value = (float)ultrasonic.distanceCm(50000);
     sendFloat(value);
   }
   break;
